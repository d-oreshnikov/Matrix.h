...
START_TEST(compliment_test_1) {
  ...
  int code = my_calc_complements(&B, &R);
  ck_assert_int_eq(my_eq_matrix(&R, &A), SUCCESS);
...
START_TEST(compliment_test_2) {
  ...
  ck_assert_int_eq(my_eq_matrix(&R, &B), 1);
  my_remove_matrix(&A);
  my_remove_matrix(&B);
  my_remove_matrix(&R);
}
...
START_TEST(compliment_test_1x1) {
  ...
  ck_assert_int_eq(my_eq_matrix(&R, &B), 0);
  my_remove_matrix(&A);
  my_remove_matrix(&B);
  my_remove_matrix(&R);
}
...
START_TEST(null_compliments) {
  matrix_t *B = NULL;
  matrix_t *R = NULL;
  int res = my_calc_complements(B, R);
...
START_TEST(create_test) {
  ...
  int columns = 10;
  matrix_t matrix = {0};
  int code = my_create_matrix(rows, columns, &matrix);
  ...
  if (code == 0) my_remove_matrix(&matrix);
}
END_TEST
...
START_TEST(determinant_not_square) {
  matrix_t A;
  double B;
  my_create_matrix(4, 5, &A);
  int res = my_determinant(&A, &B);
...
START_TEST(eq_test_2) {
  ...
  ck_assert_int_eq(my_eq_matrix(&A, &B), 1);
  my_remove_matrix(&A);
  my_remove_matrix(&B);
}
END_TEST
...
START_TEST(test_normal) {
  ...
  int code = my_inverse_matrix(&m, &R);

  ck_assert_int_eq(my_eq_matrix(&R, &B), SUCCESS);
  ck_assert_int_eq(code, 0);
  my_remove_matrix(&m);
  my_remove_matrix(&R);
  my_remove_matrix(&B);
...
START_TEST(test_not_sqare) {
  matrix_t m = {0};
  matrix_t result = {0};
  my_create_matrix(1, 4, &m);
  int code = my_inverse_matrix(&m, &result);
  ck_assert_int_eq(code, 2);
  my_remove_matrix(&m);
...
START_TEST(incorrect_matrix_1) {
  matrix_t m1 = {0};
  matrix_t m2 = {0};
  my_create_matrix(5, 5, &m1);
  my_create_matrix(6, 6, &m2);

  matrix_t result = {0};
  int mul_code = my_mult_matrix(&m1, &m2, &result);
...
START_TEST(mult_matrix_test_3) {
  matrix_t A, B, R, R2;
  my_create_matrix(3, 2, &A);
  my_create_matrix(2, 4, &B);
  my_create_matrix(3, 4, &R2);
...
START_TEST(null_mult_num) {
  ...
  double num = 0;
  matrix_t *R = NULL;
  int res = my_mult_number(B, num, R);
  ck_assert_int_eq(res, 1);
}
END_TEST
